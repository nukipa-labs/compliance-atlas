...
import { createNukipaClient, NukipaApiError, type GateFormField } from '@nukipa/site-sdk';
...
export function GateForm(props: GateFormProps) {
  ...
  const onSubmit = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    try {
      // Browser-side: NukipaApiError carries the upstream error message
      // verbatim, so we surface that to the visitor instead of a generic
      // "Submission failed". Same behaviour the raw-fetch version had,
      // just with the host-resolution + retry headers handled inside the
      // SDK.
      const client = createNukipaClient({
        gatewayUrl: GATEWAY,
        getHost: () => (typeof window !== 'undefined' ? window.location.host : ''),
        defaultRevalidate: false
      });
      await client.submitForm(props.formSlug, values);
      ...
      if (err instanceof NukipaApiError) {
